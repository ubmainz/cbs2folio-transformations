...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="permanentLocationId">
        ...
        <permanentLocationId>
            <xsl:choose>
                ...
                <xsl:when
                    test="substring($i/datafield[@tag = '208@']/subfield[@code = 'b'], 1, 1) = 'd'"
                    >DUMMY</xsl:when>
                <!-- Regeln für UB -->
                <xsl:when test="$abt = '000'">
                    <xsl:choose>
                        ...
                        <xsl:when test="starts-with($signatur, '/')"
                            >ILN204/CG/UB/Freihand1OG</xsl:when>
                        <!-- Temporäre Erwerbungssignatur - Standort so lassen?-->
                        <xsl:when test="starts-with($signatur, '000 ')"
                            >ILN204/CG/UB/Freihand1OG</xsl:when>
                        <!-- RVK-Signaturen -->
                        ...
                        <xsl:when test="starts-with($signatur, '1/')"
                            >ILN204/CG/UB/UBMagKeller</xsl:when>
                        ...
                        <!-- Alte Dissen mit EJ vorne im SLS -->
                        <xsl:when test="starts-with($signatur, '19')"
                            >ILN204/CG/UB/UBMagAltbau</xsl:when>
                        <!-- Dissen 1900-1990 im Altbau -->
                        <xsl:when test="starts-with($signatur, '2/')"
                            >ILN204/CG/UB/UBMagKeller</xsl:when>
                        <!-- KSTR Signaturen im Keller -->
                        <xsl:when test="starts-with($signatur, '20.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->
                        <xsl:when test="starts-with($signatur, '21.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->
                        <xsl:when test="starts-with($signatur, '22.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->
                        <xsl:when test="starts-with($signatur, '23.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->
                        <xsl:when test="starts-with($signatur, '24.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->
                        <xsl:when test="starts-with($signatur, '27.')"
                            >ILN204/CG/UB/UBFreihand1OG</xsl:when>
                        <!-- Arbeitsplatz Mikroformen -->
                        <xsl:when test="starts-with($signatur, '2o 1/')"
                            >ILN204/CG/UB/UBMagKeller</xsl:when>
                        <!-- Sind doch eigentlich auch KSTR -->
                        <xsl:when test="starts-with($signatur, '2o 2/')"
                            >ILN204/CG/UB/UBMagKeller</xsl:when>
                        <!-- Sind doch eigentlich auch KSTR -->
                        <xsl:when test="starts-with($signatur, '2o 20.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->
                        <xsl:when test="starts-with($signatur, '2o 21.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->
                        <xsl:when test="starts-with($signatur, '2o 22.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->
                        <xsl:when test="starts-with($signatur, '2o 23.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->
                        <xsl:when test="starts-with($signatur, '2o 24.')"
                            >ILN204/CG/UB/UBMag3</xsl:when>
                        <!-- 20er Signaturen im bis 24.999.99 in Mag3 -->

                        <!-- Für später -->
                        <xsl:when test="starts-with($signatur, 'FH ')"
                            >ILN204/CG/UB/Freihand2OG</xsl:when>
                        <xsl:when test="starts-with($signatur, '4o ')">ILN204/CD/UB/UBMag</xsl:when>
                        <xsl:when test="starts-with($signatur, 'ADk')"
                            >ILN204/CG/UB/Altbau</xsl:when>
                ...
                <xsl:when test="$abt = '002'">
                    <xsl:choose>
                        <xsl:when test="starts-with($signatur, '002')"
                            >ILN204/CG/ZNL/Freihand</xsl:when>
                ...
                <xsl:when test="$abt = '005'">
                    <xsl:choose>
                        <xsl:when test="starts-with($signatur, '005')"
                            >ILN204/CG/ZHB/Freihand</xsl:when>
    ...
    <xsl:template name="prefix">
        ...
        <xsl:message>Debug: <xsl:value-of select="$cnelement"/> Prefix "<xsl:value-of
                select="$cnprefix"/>"</xsl:message>
        ...
        <xsl:element name="{$cnelement}">
            <xsl:value-of
                select="normalize-space(translate(substring-after($cn, $cnprefix), '@', ''))"/>
        </xsl:element>
    </xsl:template>
